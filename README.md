...
- **发音没声音，或发音很难听（像机器人/外国腔）？**
  这取决于手机里“文字转语音(TTS)引擎”的质量。想要地道的美式/英式发音，请这样设置：
  1. 手机装一个 **Google 文字转语音引擎**（应用商店搜 “Google 文字转语音”/“Speech Services by Google”）。
  2. 设置 → 系统 → 语言和输入法 → **文字转语音输出**，把“首选引擎”改为 **Google**。
  3. 点引擎旁的设置 → 安装语音数据 → 下载 **English (United States)** 或 **English (United Kingdom)** 的高质量语音。
  4. 回到 App 首页，用底部“🔊 朗读口音”选择 **美式 / 英式**。
  这样单词、句子、文章的朗读就会用标准英美发音。
